...
# reaches the Go2, then launch the wander web server (which spawns the sensing+
# actuation pipeline and, on demand, the reactive wander node). Same NIC-binding
# as the nav2/recorder/slam services — self-contained, no nav2 stack.
set -e

source /opt/ros/humble/setup.bash
[ -f /unitree_ws/install/setup.bash ] && source /unitree_ws/install/setup.bash

# Declared as a WendyOS `frameworks.ros2` service (wendy.json): the agent injects
# ROS_DOMAIN_ID (0) + RMW for us, but also ROS_LOCALHOST_ONLY=1 and an
# interface-less CYCLONEDDS_URI (Wendy graphs are loopback-only). The Go2 is a
# PHYSICAL robot on the LAN, so undo both and re-bind to our NIC below.
...
echo "[wander] Go2=${GO2_IP}  iface=${IFNAME:-auto}  local=${LOCALIP:-auto}  domain=${ROS_DOMAIN_ID}"
