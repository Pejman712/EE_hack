...
MOVE_DIRECTIONS = {
    ...
    "backward": {"x": -MOVE_SPEED_MPS, "y": 0.0, "z": 0.0},
    "left": {"x": 0.0, "y": MOVE_SPEED_MPS, "z": 0.0},
    "right": {"x": 0.0, "y": -MOVE_SPEED_MPS, "z": 0.0},
}
...
def api_go2_sit() -> dict:
    return _publish_sport_request(SPORT_API_ID_SIT)


@app.post("/api/go2/stand")
def api_go2_stand() -> dict:
...
def api_go2_move(direction: str) -> dict:
    ...
    if params is None:
        raise HTTPException(404, f"unknown move direction: {direction}")
    return _publish_sport_request(SPORT_API_ID_MOVE, params)


@app.post("/api/go2/stop")
def api_go2_stop() -> dict:
    return _publish_sport_request(SPORT_API_ID_STOP)


def _publish_sport_request(api_id: int, params: dict | None = None) -> dict:
    msg = {"header": {"identity": {"api_id": api_id}}}
    if params is not None:
        msg["parameter"] = json.dumps(params, separators=(",", ":"))
    payload = json.dumps(msg, separators=(",", ":"))
    try:
        out = subprocess.run(
            ["ros2", "topic", "pub", "--once", SPORT_REQUEST_TOPIC, SPORT_REQUEST_TYPE, payload],
            capture_output=True,
            text=True,
            timeout=10,
        )
    except subprocess.TimeoutExpired as e:
        raise HTTPException(504, f"timed out publishing sport request to {SPORT_REQUEST_TOPIC}") from e
    except Exception as e:
        raise HTTPException(500, f"failed to publish sport request: {e}") from e
    if out.returncode != 0:
        detail = (out.stderr or out.stdout or "ros2 topic pub failed").strip()
        raise HTTPException(500, detail)
    return {
        "ok": True,
        "topic": SPORT_REQUEST_TOPIC,
        "type": SPORT_REQUEST_TYPE,
        "api_id": api_id,
        "params": params,
    }
...
<style>
  ...
  *{box-sizing:border-box;} body{margin:0;background:var(--bg);color:var(--ink);
  ...
  .drive .wide{grid-column:auto;}
  .drive .spacer{height:1px;}
  ...
  <div class="drive">
    ...
    <button class="move" id="moveForwardBtn" onclick="move('forward')">Forward</button>
    <div class="spacer"></div>
    <button class="move" id="moveLeftBtn" onclick="move('left')">Left</button>
    <button class="move stopmove" id="moveStopBtn" onclick="stopMove()">Stop</button>
    <button class="move" id="moveRightBtn" onclick="move('right')">Right</button>
    <div class="spacer"></div>
    <button class="move" id="moveBackwardBtn" onclick="move('backward')">Backward</button>
    <div class="spacer"></div>
  </div>
...
<script>
  ...
  async function move(direction){
    const ids={forward:"moveForwardBtn",backward:"moveBackwardBtn",left:"moveLeftBtn",right:"moveRightBtn"};
    await pulseButton(ids[direction],()=>j("/api/go2/move/"+direction,"POST"));
  }
  async function stopMove(){
    await pulseButton("moveStopBtn",()=>j("/api/go2/stop","POST"));
  }
