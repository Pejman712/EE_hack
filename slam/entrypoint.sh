...
# reaches the Go2, then launch the SLAM pipeline. Same NIC-discovery approach as
# recorder/entrypoint.sh (this service is self-contained — it doesn't rely on
# WendyOS's frameworks.ros2 DDS injection).
set -e
...
echo "[slam] Go2=${GO2_IP}  iface=${IFNAME:-auto}  local=${LOCALIP:-auto}  domain=${ROS_DOMAIN_ID}"
...
mkdir -p /maps
exec ros2 launch /slam.launch.py
